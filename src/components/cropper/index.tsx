...
import './index.less';
import {uploadLogoApi} from "@/api/modules/oss";
import {useDispatch, useSelector} from "react-redux";
import {State} from "@/types/redux";
import {setUser} from "@/redux/modules/global";
...
const CropperComponent = () => {
    ...
    const handleFileChange = (e) => {
      ...
      if (file) {
          ...
          if (file.size / 1024 <= 5 * 1024) {
              ...
              openNotificationWithIcon(notificationContext,"文件过大", "所选文件不能超过5M")
          }
      } else {
          openNotificationWithIcon(notificationContext, "上传提示", "请选择文件");
      }
      e.target.value = ''
    };
    ...
    const upload = async (image) => {
        setConfirm(true)
        //这边写图片的上传
        let para:Logo.ReqLogo = {
            content: image.toString()
        };
        try {
            const {msg, code, data} = await uploadLogoApi(para);
            setConfirm(false)
            if (code == 0) {
                openNotificationWithIcon(notificationContext, "上传结果", "上传成功","success");
                // 覆写到本地缓存
                const _user:User.ResUser = deepClone(user);
                _user.logo = data
                dispatch(setUser(_user))
                setHeaderImage(data)
            } else {
                openNotificationWithIcon(notificationContext, "上传结果", msg);
            }
        }catch (e) {
